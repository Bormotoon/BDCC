...
func generateCharacterID() -> String:
	if(ServiceLocator.safe_get_service(&"MainScene").getEncounterSettings().doesPreferKnownEncounters()):
		return ""
...
func characterIDCanBePicked(_theCharID:String) -> bool:
	if(ServiceLocator.safe_get_service(&"MainScene").IS.wasCharIDUsedToday(_theCharID)):
		return false
	if(ServiceLocator.safe_get_service(&"MainScene").IS.hasPawn(_theCharID)):
		return false
	if(ServiceLocator.safe_get_service(&"MainScene").characterIsVisible(_theCharID)):
		return false
...
func tryPickCharacterID() -> String:
	...
	var possibleCharIds:Array = ServiceLocator.safe_get_service(&"MainScene").getDynamicCharacterIDsFromPool(pool)
...
func onPawnMeetWith(_pawn, _otherPawn, _otherPawnMoved:bool) -> bool:
	return ServiceLocator.safe_get_service(&"MainScene").IS.checkOnMeetInteractions(_pawn, _otherPawn, _otherPawnMoved)
