...
static func colorInterpolateStr(col1, col2, weight:float) -> String:
	...
	#return col1.lerp(col2, weight).to_html()
	return Color.from_hsv(col1.h*(1.0-weight)+col2.h*weight, col1.s*(1.0-weight)+col2.s*weight, col1.v*(1.0-weight)+col2.v*weight).lerp(col1.lerp(col2, weight), 0.7).to_html()
