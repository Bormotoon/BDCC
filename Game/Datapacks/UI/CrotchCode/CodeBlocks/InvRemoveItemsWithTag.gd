...
func execute(_contex:CodeContex):
	...
	if(op == "anywhere"):
		_contex.charInventoryMethod(charName, "removeItemsWithTag", [actualTag])
		_contex.charInventoryMethod(charName, "removeEquippedItemsWithTag", [actualTag])
	if(op == "in inventory"):
		return _contex.charInventoryMethod(charName, "removeItemsWithTag", [actualTag])
	if(op == "equipped"):
		return _contex.charInventoryMethod(charName, "removeEquippedItemsWithTag", [actualTag])

func getTemplate():
	return [
		{
			type = "label",
			text = "Remove items with tag",
		},
		{
			type = "slot",
			id = "tagSlot",
