...
func _init():
	...
	valSlot.setRawValue(Buff.BlindfoldBuff)
...
func execute(_contex:CodeContex):
	...
	if(!isString(statName)):
		throwError(_contex, "Buff name must a string, got "+str(statName)+" instead")
		return	
		
	return _contex.charMethod(charName, "hasBuff", [statName])
...
func updateVisualSlot(_editor, _id, _visSlot):
	...
	if(_id == "val"):
		_visSlot.setPossibleValues(GlobalRegistry.getBuffClasses().keys())
