...
func _init():
	...
	valSlot.setRawValue("caninepenis")
...
func execute(_contex:CodeContex):
	...
	if(!isString(statName)):
		throwError(_contex, "Bodypart id must a string, got "+str(statName)+" instead")
		return	
		
	return _contex.charMethod(charName, "switchBodypartSimple", [statName])
...
func updateVisualSlot(_editor, _id, _visSlot):
	...
	if(_id == "val"):
		_visSlot.setPossibleValues(GlobalRegistry.getBodypartRefs().keys())
