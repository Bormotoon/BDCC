...
func execute(_contex:CodeContex):
	...
	if(op == "anywhere"):
		return _contex.charInventoryMethod(charName, "getItemsWithTagCount", [actualTag]) + _contex.charInventoryMethod(charName, "getEquippedItemsWithTagCount", [actualTag])
	if(op == "in inventory"):
		return _contex.charInventoryMethod(charName, "getItemsWithTagCount", [actualTag])
	if(op == "equipped"):
		return _contex.charInventoryMethod(charName, "getEquippedItemsWithTagCount", [actualTag])

func getTemplate():
	return [
		{
			type = "label",
			text = "Get amount of items with tag",
		},
		{
			type = "slot",
			id = "tagSlot",
