...
func _reactInit():
	if(ServiceLocator.safe_get_service(&"EventSystem").triggerReact(Trigger.ApproachedYogaMats)):
		endScene()
		return
...
func _run():
	...
	if(state == ""):
		...
		ServiceLocator.safe_get_service(&"EventSystem").triggerRun(Trigger.ApproachedYogaMats)
