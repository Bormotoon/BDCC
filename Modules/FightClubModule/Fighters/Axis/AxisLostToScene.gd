...
func _run():
	...
	if(state == ""):
		...
		ServiceLocator.safe_get_service(&"EventSystem").triggerRun("ArenaFighterPCLost", ["axis"])
...
func _react(_action: String, _args):
	...
	if(_action == "get_fucked"):
		if(ServiceLocator.safe_get_service(&"Player").hasVagina()):
			ServiceLocator.safe_get_service(&"Player").gotVaginaFuckedBy("axis")
			ServiceLocator.safe_get_service(&"Player").cummedInVaginaBy("axis")
		else:
			ServiceLocator.safe_get_service(&"Player").gotAnusFuckedBy("axis")
			ServiceLocator.safe_get_service(&"Player").cummedInAnusBy("axis")
		ServiceLocator.safe_get_service(&"Player").orgasmFrom("axis")
		ServiceLocator.safe_get_service(&"Player").addSkillExperience(Skill.SexSlave, 30, "axis_getfucked")
