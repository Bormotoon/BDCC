...
func _init():
	sceneID = "NurseCaughtOfflimitsScene"
	...
	if(state == ""):
		...
		if(!sawBefore):
			...
			var possibleLines = [
				...
				"[say=npc]You got lost, patient?[/say]",
			...
			if(npcVariation == "mean"):
				possibleLines = [
					...
					"[say=npc]I see a stray patient. Don’t recall your face. But you’re in huge trouble.[/say]",
					"[say=npc]Patients never learn, do they. Well, more thugs for me to beat up.[/say]",
			...
			if(npcVariation == "kind"):
				possibleLines = [
					...
					"[say=npc]You can’t be here, patient. Don’t be silly~.[/say]",
			...
			if(npcVariation == "subby"):
				possibleLines = [
					"[say=npc]Ohh.. Hi there, patient..[/say]",
		...
		# (old guard)
			...
			var possibleLines = [
				...
				"[say=npc]Got lost again, patient?[/say]",
			...
			if(npcVariation == "mean"):
				possibleLines = [
					...
					"[say=npc]I remember you, patient. But you’re in huge trouble.[/say]",
		...
		saynn("You turn around and see one of the nurses, judging by {npc.his} uniform.")
...
#		addButton("Submit", "Let them look through your things", "submit")
	...
	if(state == "attack"):
		...
		# (if subby/coward happened)
			...
			saynn("{npc.He} totally wasn’t prepared for that. The nurse gasps and collapses before you.")
			...
			saynn("{npc.He} staggers back but quickly recovers. The nurse doesn’t look happy with you.")
	...
	if(state == "if_won"):
		saynn("You pin the defeated nurse to the floor. {npc.He} doesn’t seem to struggle much.")

		saynn(RNG.pick([
			"[say=npc]What’s your plan?[/say]",
			"[say=npc]Don’t leave me like this, patient.[/say]",
	...
	if(state == "offer_body"):
		saynn("You really wanna avoid any fights. You offer the nurse a kind smile while striking a sexy pose, showing off your {pc.masc} curves.")

		saynn("[say=pc]How about I offer you something? Myself for example.[/say]")
		...
		# (if subby)
			...
			saynn("{npc.He} blushes while sneakily glancing at you. The shy nurse lowers {npc.his} head.")

			saynn("[say=npc]I mean.. You can take me if you want then..[/say]")

			saynn("Huh, this nurse seems to be quite subby.")
			addWonButton()
		else:
			# (if default)
	...
	if(state == "after_sex" || state == "after_sex_won"):
		saynn("After the fun time ends, the nurse just leaves you alone.")
	...
	if(state == "catchEscapedPC"):
		...
		saynn("[say=npc]Oh, I know who you are. Thought you can escape, test bitch?[/say]")

		saynn("{npc.He} forces a straitjacket on you! You try to resist but it’s too late.")

		saynn("[say=npc]Not so fast, you’re coming with me.[/say]")

		saynn("{npc.He} leashes you and brings you to Eliza. The doctor doesn’t seem to be happy about you.")
