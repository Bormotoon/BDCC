...
func _add_bone_tracks( animation:Animation ):
	...
	for handle_bone in self.bone_handle_nodes:
		var path:NodePath = NodePath( "%s:%s" % [ (animation_player.get_node( self.animation_player.root_node ).get_path_to( self.skeleton )), handle_bone.name ] )
